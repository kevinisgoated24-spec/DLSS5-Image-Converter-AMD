...
// specific, actionable report of what is missing and where to get it, instead of a confusing
...
bool AskYesNo(const std::string& question) {
    printf("%s: %s [y/N] ", kName, question.c_str());
    fflush(stdout);
    char line[16] = {};
...
// Returns true only if everything needed to actually run a conversion is present. False means
// the caller should stop rather than let a missing file surface as a confusing failure later.
bool CheckDependencies() {
    ...
    if (!ffmpegOk) {
        if (AskYesNo("ffmpeg was not found on PATH. Download a portable copy automatically?")) {
            if (DownloadFfmpeg()) {
                PrependToPath(localFfmpegBin);
                ffmpegOk = CheckFfmpegOnPath();
            }
        } else {
            printf("%s: ffmpeg is required. Install it yourself and put it on PATH, or run this "
                   "again and say yes.\n", kName);
        }
    }
    ...
    if (!missing.empty()) {
        ...
        printf("%s: see the README for full setup instructions.\n", kName);
    }
...
int main(int argc, char** argv) {
    ...
    if (!CheckDependencies()) {
        printf("%s: cannot continue until the above is resolved\n", kName);
        return 1;
    }
